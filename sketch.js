...
function preload() {
  ...
  waterSplashingSound = loadSound("assets/cannon_water.mp3");
...
function keyPressed() {
  if (keyCode === DOWN_ARROW) {
    ...
    cannonExplosionSound.setVolume(0.5);
...
function showCannonBalls(ball, index) {
  ...
  if (ball.body.position.x >= width || ball.body.position.y >= height - 50) {
    if (!ball.isSink) {
      ball.remove(index);
  waterSplashingSound.play();
  waterSplashingSound.setVolume(0.5);
    }
  }
}
  ...
  if (boats.length > 0) {
    ...
    for (var i = 0; i < boats.length; i++) {
      ...
      if (collision.collided && !boats[i].isBroken) {
        ...
        pirateLaughingSound.setVolume(0.5);
      }
    }
  } else {
